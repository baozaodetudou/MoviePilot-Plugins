...
本文档面向 `v1.4+`，用于快速理解三种触发模式、关键判断关系、参数影响与日志排查方式。

## 0. 近期版本变更

- `v1.4`
  - 重构“立即清理”交互：
  - 保留三个入口（配置页 / 数据页固定按钮 / 运行总览按钮）。
  - 三个入口统一为直接调用 `clean` 接口。
  - 移除 `iframe` 跳转与自动打开日志弹窗逻辑，避免界面跳转干扰。
- `v1.3`
...
- `v1.0`
  - 删除目标升级为目录级：
  - 电影：按电影所在目录删除。
  - 电视剧：按“当前识别季目录”删除（不会整剧删除）。
  - 删除流程改为“先扫描后删除”：
  - 先收集删除目标（含硬链接关联文件）。
  - 再统一执行删除（先文件、后目录）。
  - 硬链接兜底支持目录级扫描：
  - 对目录内媒体文件提取 inode，再跨根目录匹配外部硬链接文件。
  - 命中后统一纳入删除计划。
